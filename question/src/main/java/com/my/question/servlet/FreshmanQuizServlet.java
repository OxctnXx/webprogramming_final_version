...
public class FreshmanQuizServlet extends HttpServlet {
    @Override
    public void doGet(HttpServletRequest request, HttpServletResponse response) {
        ResultSet resultSet = JDBCUtils.executeQuery("select * from  `freshman_quiz` ");
        try {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) {
        ...
        ResultSet resultSet = JDBCUtils.executeQuery("select * from  `freshman_quiz` where id=?", id);
        try {
            if (resultSet.next()) {
